...
import { submitAutopost, previewAutopost } from './actions';
...
export class AutopostForm extends Component {
  ...
  static propTypes = {
    ...
    autopost: PropTypes.string.isRequired,
  ...
  constructor(props) {
    super(props);
    const { autopost, version, siteLogo } = this.props;
    let { organizations } = this.props;
    this.autopost = JSON.parse(autopost);
    ...
    const isLocalstorageNewer =
      new Date(previousContent.updatedAt) > new Date(this.autopost.updated_at);
    ...
    this.state = {
      ...
      id: this.autopost.id || null, // eslint-disable-line react/no-unused-state
      title: this.autopost.title || '',
      tagList: this.autopost.cached_tag_list || '',
      ...
      previewResponse: '',
      submitting: false,
      editing: this.autopost.id !== null, // eslint-disable-line react/no-unused-state
      mainImage: this.autopost.main_image || null,
      organizations,
      organizationId: this.autopost.organization_id,
      errors: null,
      edited: false,
      updatedAt: this.autopost.updated_at,
      version,
      siteLogo,
      helpFor: null,
      helpPosition: null,
  ...
  fetchPreview = (e) => {
    ...
    if (previewShowing) {
      ...
      previewAutopost(bodyMarkdown, this.showPreview, this.failedPreview);
    }
  };
  ...
  onPublish = (e) => {
    ...
    submitAutopost({
      ...
      onError: this.handleAutopostError,
    });
  };
