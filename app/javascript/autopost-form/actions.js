...
export function previewAutopost(payload, successCb, failureCb) {
  fetch('/autoposts/preview', {
    method: 'POST',
    headers: {
...
export function submitAutopost({ payload, onSuccess, onError }) {
  const method = payload.id ? 'PUT' : 'POST';
  const url = payload.id ? `/autoposts/${payload.id}` : '/autoposts';
  fetch(url, {
    method,
    headers: {
      Accept: 'application/json',
    ...
    body: JSON.stringify({
      autopost: processPayload(payload),
    }),
    credentials: 'same-origin',
  })
