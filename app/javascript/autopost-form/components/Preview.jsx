...
function titleArea({
  ...
  const tagArray = previewResponse.tags || autopostState.tagList.split(', ');
  let tags = '';
  ...
  // The v2 editor stores its cover image in autopostState.mainImage, while the v1 editor
  // stores it as previewResponse.cover_image. When previewing, we handle both by
  // defaulting to setting the cover image to the mainImage on the autopost (v2),
  //  and only using the cover image from the previewResponse if it exists (v1).
  let coverImage = autopostState.mainImage || '';
  if (autopostState.previewShowing) {
  ...
  const previewTitle = previewResponse.title || autopostState.title || '';

  return (
    <header className="crayons-article__header">
      {coverImage.length > 0 && (
        <div
          data-testid="autopost-form__cover"
      ...
      <div className="crayons-article__header__meta">
        ...
        <h1 className="fs-3xl m:fs-4xl l:fs-5xl fw-bold s:fw-heavy lh-tight mb-2 spec-autopost__title">
          {previewTitle}
        </h1>

        <div className="spec-autopost__tags color-base-60">{tags}</div>
      </div>
    </header>
  );
}
...
export const Preview = ({
  ...
  return (
    <div className="crayons-article-form__content crayons-card">
      <autopost className="crayons-article">
        {titleArea({
          previewResponse,
          autopostState,
          errors,
