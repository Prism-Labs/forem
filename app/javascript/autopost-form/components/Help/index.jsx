...
import { AutopostFormTitle } from './AutopostFormTitle';
 ...
 * @param {string} props.version Version of the editor used for autopost
 *
...
export const Help = ({ previewShowing, helpFor, helpPosition, version }) => {
  ...
  return (
    <div className="crayons-article-form__aside">
      {!previewShowing && (
        <div
          data-testid="autopost-form__help-section"
          className="sticky"
          style={{ top: version === 'v1' ? '56px' : helpPosition }}
        >
          {helpFor === 'autopost-form-title' && <AutopostFormTitle />}
          {helpFor === 'tag-input' && <TagInput />}

          {version === 'v1' && <BasicEditor openModal={openModal} />}

          {(helpFor === 'autopost_body_markdown' || version === 'v1') && (
