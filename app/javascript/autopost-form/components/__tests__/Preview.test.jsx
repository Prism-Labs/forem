...
describe('<Preview />', () => {
  ...
  it('should have no a11y violations', async () => {
    const { container } = render(
      <Preview
        ...
        autopostState={getAutopostState()}
        errors={errors}
      />,
    );
  ...
  it('shows a cover image in the preview if one exists', () => {
    const autopostState = { ...getAutopostState(), previewShowing: true };
    const { getByTestId, getByAltText } = render(
      <Preview
        previewResponse={getPreviewResponse()}
        autopostState={autopostState}
        errors={errors}
      />,
    );
    const coverImage = getByAltText(/post preview cover/i);

    getByTestId('autopost-form__cover');
  ...
  it('does not show a cover image in the preview if one does not exist', () => {
    const autopostState = { ...getAutopostState(), mainImage: null };
    ...
    expect(queryByTestId('autopost-form__cover')).toBeNull();
  });
