...
import { AutopostCoverImage } from '../AutopostCoverImage';

global.fetch = fetch;

describe('<AutopostCoverImage />', () => {
  beforeEach(() => {
    global.Runtime = {
      isNativeIOS: jest.fn(() => {
        return false;
  ...
  it('displays an upload input when there is no main image', () => {
    const { getByLabelText } = render(
      <AutopostCoverImage mainImage="" onMainImageUrlChange={jest.fn()} />,
    );
  ...
  describe('when an image is uploaded', () => {
    ...
    it('removes an existing cover image', async () => {
      ...
      // This is handled in the autopost <Form /> component.
    });
